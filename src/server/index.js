...
class TabRouter {
  constructor({ event, defaultRes, callback }) {
    ...
    this._req = { event, data: event.data, url: data.url };
    this._res = { callback };
  }
  receive(path, handle) {
    // 这个东西是否需要
  ...
  use(path, handle) {
    ...
    this._middlewares.push({ handle, path });
  }
  apply() {
    // 相当于http来了
    let index = 0;
    const next = err => {
      ...
      if (err) {
        ...
        const { path, handle } = this._middlewares[index++];
        if (path === "*") {
          handle(this._req, this._res, next);
        } else if (new RegExp(`^${this._req.url}$`).test(path)) {
          handle(this._req, this._res, next);
        } else {
          next();
        }
